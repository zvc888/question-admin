...
public class JwtFilter extends BasicHttpAuthenticationFilter {
    ...
    public JwtFilter(JwtProperties jwtProperties, ISyncCacheService syncCacheService, JedisUtils jedisUtils, UserService userService) {
...
//        logger.info("调用executeLogin验证登录");
        ...
        //绑定上下文
        new UserContext(new LoginUser(account, user.getId(), user.getName()));
...
//            return true;
        ...
        if (isLoginAttempt(request, response)) {
            try {
                boolean allow = this.executeLogin(request, response);
