...
import com.question.admin.scheduler.listener.QuestionJobListener;
import org.quartz.*;
...
public class QuestionSchedulerService {
	...
	private static final String JOB_GROUP = "QUESTION_JOB_GROUP";
	private static final String TRIGGER_GROUP = "QUESTION_TRIGGER_GROUP";
	
	@Autowired
	@Qualifier("questionSchedulerFactory")
	private SchedulerFactoryBean schedulerFactory;

	@Autowired
	private QuestionJobListener questionJobListener;
	...
	public void addJob(String jobId, String jobClazz, String cron, Date startTime, boolean startJob) {
		try {
			...
			scheduler.getListenerManager().addJobListener(questionJobListener);
			if(!startJob) {
				pauseJob(jobId);
			}
		} catch (Exception e) {
